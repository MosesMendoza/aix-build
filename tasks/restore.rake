...
def bos_inst_restore(args = {})
  ...
  #
  ready_file = File.join("#{@base_dir}/.ready_#{os_ver}")
  ...
  #
  cmd = ["sudo su - #{nim_local_user} -c \"ssh -o StrictHostKeyChecking=no -t #{nim_remote_user}@#{nim_master} 'nim -o bos_inst"]
...
# nim_local_user, and type are either static or global, hence they're
...
@machine_names.each do |host|
  ...
  task "restore-#{host}" do
    ...
    args = {
      ...
      :nim_remote_user  => @nim_remote_user,
      :nim_local_user   => @nim_local_user,
